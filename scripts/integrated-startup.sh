...
load_environment_variables() {
    ...
    log "🔍 Verificando configuración de GitHub Auth:"
    if [ -z "$AUTH_GITHUB_CLIENT_ID" ]; then
        log_error "AUTH_GITHUB_CLIENT_ID no está definido en $env_file"
        return 1
    else
        log "   ✅ AUTH_GITHUB_CLIENT_ID: ${AUTH_GITHUB_CLIENT_ID:0:15}..."
    fi
    
    if [ -z "$AUTH_GITHUB_CLIENT_SECRET" ]; then
        log_error "AUTH_GITHUB_CLIENT_SECRET no está definido en $env_file"
        return 1
    else
        log "   ✅ AUTH_GITHUB_CLIENT_SECRET: ${AUTH_GITHUB_CLIENT_SECRET:0:15}..."
    fi
    
    if [ -z "$GITHUB_TOKEN" ]; then
        log_error "GITHUB_TOKEN no está definido en $env_file"
        return 1
    else
        log "   ✅ GITHUB_TOKEN: ${GITHUB_TOKEN:0:15}..."
    fi
    
    if [ -z "$AUTH_GITHUB_CALLBACK_URL" ]; then
        log_error "AUTH_GITHUB_CALLBACK_URL no está definido en $env_file"
        return 1
    else
        log "   ✅ AUTH_GITHUB_CALLBACK_URL: $AUTH_GITHUB_CALLBACK_URL"
    fi
    
    log "✅ Variables de entorno cargadas correctamente desde $env_file"
}
...
start_ia_ops() {
    ...
    # Cargar variables de entorno antes de iniciar Backstage
    if ! load_environment_variables; then
        log_error "Error cargando variables de entorno"
        return 1
    fi
    ...
    # 4. Crear archivo de entorno local para Backstage
    log "📝 Creando archivo de entorno local para Backstage..."
    cat > .env.local << EOF
# Variables de entorno para Backstage (generado automáticamente por integrated-startup.sh)
...
EOF
    ...
    # 5. Iniciar servicios robustos
    log "🚀 Iniciando Backstage con variables de entorno cargadas..."
    ./start-robust.sh &
...
main() {
    ...
    log "📋 Paso 0: Cargando variables de entorno globales..."
    if ! load_environment_variables; then
        log_error "Error crítico: No se pudieron cargar las variables de entorno"
        exit 1
    fi
